...
public static class Utilities
{
	...
	public static string GetTableData (DataTable table)
	{
		...
		foreach (DataColumn column in table.Columns)
		{
			...
			stbColumnFormat.Append(column.ColumnName).Append("{" + columnCount.ToString() + "}");
		...
		stbTable.AppendFormat(stbColumnFormat.ToString(), GetHeaderPadStrings(dt, columnEndPaddings));
		for (int rCount = 0; rCount < lstRows.Count; rCount++)
		{
			stbTable.AppendFormat(lstRows[rCount], GetColumnPadStrings(dt.Rows[rCount], columnEndPaddings));
			stbTable.AppendLine();
		}
